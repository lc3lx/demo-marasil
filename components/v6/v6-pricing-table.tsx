...
export function V6PricingTable() {
  ...
  return (
    <Tabs defaultValue="domestic" className="w-full" onValueChange={setActiveTab}>
      <TabsList className="grid w-full max-w-md grid-cols-2 rounded-xl v6-clay border-none">
      ...
      <TabsContent value="domestic" className="mt-4">
        <div className="rounded-xl v6-clay overflow-hidden">
          <div className="grid grid-cols-4 gap-4 border-b bg-white/50 p-4 font-medium">
            <div>من</div>
            <div>إلى</div>
            <div>السعر</div>
            <div>وقت التوصيل</div>
          ...
          <div className="divide-y">
            {domesticRoutes.map((route, i) => (
              <div
                key={i}
                className="grid grid-cols-4 gap-4 p-4 hover:bg-white/50 transition-colors v6-fade-in v6-stagger-item"
        ...
        <Button variant="outline" className="mt-4 w-full rounded-xl v6-clay-button">
          عرض جميع الأسعار
        </Button>
      </TabsContent>
